...
The rule is stated where the recursion lives (`applyAction`/`checkResolution` in `resolve.ts`) and decided as **D-096**, four clauses: terminal state evaluates at node boundaries post-order, never mid-node; a fired card is one atomic batch (the `deferTerminalCheck` exception, now named rather than incidental); after a mid-tree ending every remaining action is refused with a recorded fact; and mutual zero is a **victory** — the Boss is asked first, because the raid promise is that the kill counts.
