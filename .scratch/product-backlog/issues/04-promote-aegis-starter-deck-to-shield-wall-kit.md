...
Replace the live/default Aegis Guardian starter deck with the documented twenty-card Shield Wall list:
...
Player-facing promise: the default Aegis deck should now let players naturally encounter the full first-pass tank loop in live play — defend the line, earn a visible opening, spend that opening with `Shield Slam`, and make at least one optional branch decision beyond repeating the same attack every turn.
...
- Change the live/default Aegis Guardian starter deck to the approved five-identity, twenty-card Shield Wall list.
...
- No Interception, multi-Hero targeting, or other deferred Aegis scope.
...
2. The previous evaluation-only controlled configuration is no longer the only place where that five-identity list exists; the default playable Aegis deck now reflects it.
3. Canonical content documents that describe the live/default Aegis starter deck are updated so they no longer describe the old two-card shell as current.
...
| Area | Product impact |
| --- | --- |
| Design | Owns the starter-deck intent, the live/default list, and any canonical wording that explains why this is now the active Aegis shell. |
...
- [Aegis Guardian design](../../../docs/content/heroes/aegis-guardian-design.md): controlled twenty-card Shield Wall list and role intent.
- [Aegis Guardian starter deck](../../../docs/content/decks/aegis-guardian-starter.md): current live/default deck document that will need to stop describing the two-card shell as current.
...
- [Controlled Aegis test-deck evaluation proposal](03-controlled-aegis-test-deck-evaluation.md): prior evaluation-only boundary that this new user-approved outcome supersedes for the live/default deck.
...
- The user now explicitly wants the live/default Aegis starter deck changed now, not merely evaluated in a separate non-default cohort.
...
On 2026-08-13, after reviewing the evaluation-only limitation, the user explicitly chose the broader product outcome: replace the live/default Aegis starter deck now and send that change to the Orchestrator to handle. PM records this as a separate approved intake so delivery does not silently reinterpret proposal 03.
