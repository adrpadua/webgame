...
func _init() -> void:
	...
	slot.bind(0, {"top_card": card, "charges": [charge], "project_intent": &"charge"}, true, true, &"quick")
	...
	tooltip.queue_free()
	slot.bind(0, {"top_card": card, "charges": [], "project_intent": &"replace"}, true, true, &"loadout")
	_assert(slot._get_drop_intent_label() == "REPLACE", "A projected replace intent should advertise Replace.")
	slot.bind(0, {"top_card": null, "charges": [], "project_intent": &"load"}, true, true, &"quick")
	_assert(slot._get_drop_intent_label() == "LOAD", "A projected load intent should advertise Load.")
	slot.queue_free()
