...
func _initialize() -> void:
	...
	_assert(mobile_help_button.visible and not mobile_help_pane.visible, "Full guide text should start hidden behind the help button.")
	mobile_help_button.pressed.emit()
	await _wait_frames(1)
	_assert(mobile_help_pane.visible, "Tapping help should show the guide pane.")
	mobile_help_button.pressed.emit()
	await _wait_frames(1)
	_assert(not mobile_help_pane.visible, "Tapping help again should hide the guide pane.")
