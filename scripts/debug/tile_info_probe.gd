...
func _initialize() -> void:
	...
	var status_rect: Rect2 = main.get_node("%MobileStatus").get_global_rect()
	var pane_rect := pane.get_global_rect()
	_assert(pane_rect.position.y >= status_rect.end.y + 6.0 - 0.1, "Mobile tile HUD should stay below the reserved status lane.")
	_assert(Rect2(Vector2.ZERO, main.size).encloses(pane_rect), "Mobile tile HUD should stay fully inside the portrait viewport.")
