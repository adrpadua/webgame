...
func _load_slot(engine, action) -> void:
	...
	if hero.is_empty() or card == null or not hero["hand"].has(card):
		...
		return
	if hero["stamina"] < 1 or slot_index < 0 or slot_index >= hero["action_bar"].size():
		action.resolve_failure("Loading a Slot requires one Stamina and a legal Slot.")
		return
	var slot: Dictionary = hero["action_bar"][slot_index]
	if slot["top_card"] != null and not slot["charges"].is_empty():
		action.resolve_failure("A charged Slot cannot be replaced.")
		return
	hero["stamina"] -= 1
...
func _charge_slot(engine, action) -> void:
	...
	if hero.is_empty() or card == null or not hero["hand"].has(card):
		...
		return
	if hero["stamina"] < 1 or slot_index < 0 or slot_index >= hero["action_bar"].size():
		action.resolve_failure("Charging requires one Stamina and a legal Slot.")
		return
	var slot: Dictionary = hero["action_bar"][slot_index]
...
func _fire_slot(engine, action) -> Array:
	...
	if slot["activated_window"] == engine.phase:
		...
		return []
	if card.get_window_speed() != engine.phase or hero["energy"] < card.cost:
		action.resolve_failure("The Top Card cannot fire in this window.")
		return []
	if card.damage > 0 and action.payload.get("target_id", &"") == &"":
		action.resolve_failure("The Top Card needs a target.")
		return []
	var effects := card_resolver.resolve_fire(card, slot["charges"])
	hero["energy"] -= effects["energy_cost"]
	hero["energy"] += effects["energy_delta"]
	...
	if effects["target_damage"] > 0:
		...
		followups.append(EncounterActionModel.damage(action.source_id, target_id, effects["target_damage"], card.title))
...
func _move_hero(engine, action) -> Array:
	...
	if hero.is_empty() or engine.phase != &"quick" or hero["stamina"] < 1:
		action.resolve_failure("Hero movement requires the Quick Window and one Stamina.")
		return []
...
func _spawn_minion(engine, action) -> void:
	...
	var coords: Vector2i = action.payload.get("coords", Vector2i(999, 999))
	var health: int = action.payload.get("health", 2)
