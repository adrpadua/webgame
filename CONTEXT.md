...
**Status Icon**:
One live `Counter` as a raked square rather than a word (D-088, party-frame direction 1A): the Counter's mark cut in the material its channel names, on the same 8-degree plate every other HUD surface wears, with the count in the bottom-right corner whenever more than one is held and a clock across the foot while a Counter with an authored duration runs down. Icons stand in one tray beside the readout of whichever piece holds the Counters — the `Hero Frame` for the Hero, the `Stat Panel` for a Boss or a Minion — where an MMO puts its buff tray, packed at 28pt with the tray as a whole taking the 44pt target and wrapping upward as it fills. Reading a mark is a glance; the tray's hold opens one `Detail Popup` naming every Counter in it, each row led by the same square, with its numbers and its authored rules text. One tray rather than one control per Counter for the reason the `Hero Frame` is one hold target rather than four: a mark that owed its own 44pt would price a Party's afflictions in chrome the 390pt surface does not have. The mark is presentation keyed by Counter id and never authored in `data/counters/`; a Counter nobody has drawn yet falls back to a neutral steel mark rather than failing to load. Presentation only: what a Counter does is the Counter's, and the square never changes it.
