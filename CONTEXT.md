...
**Downed**:
The state of a Hero at `health <= 0`. It is *derived from the number and never stored*, so nothing can record a state the health disagrees with. A Downed Hero remains on its hex as a blocking, non-targetable body and keeps the Counters it holds, because the body is still on the board. Its Slots and earned Charges **freeze** — loaded cards stay tucked, Charges stay counted, every action but the diminished one is refused — and its hand **no longer refills**, so the hand it fell with is its whole budget on the floor. Being Downed does not satisfy anything the Party is asked for: a Downed Hero is not selectable by a Boss Beat's Role selector, and does not answer the proximity demand — a body cannot answer a demand. **The state is stable.** There is no window, no timer and no expiry; it ends only by `Revive` or with the Encounter. Each Round its player spends one card from hand on one **Diminished Action**. The Encounter is lost when every seated Hero is Downed at once, which makes a Party of one that falls an immediate defeat without a rule saying so (D-079, ADR 0040).
_Avoid_: Dead, eliminated, permanently defeated, incapacitated, downed timer

**Diminished Action**:
One of the three ally-facing actions a `Downed` Hero's player may take each Round, costing one card from their hand: `grant_ally_armor`, `ally_draws_card`, `reduce_escalation`. The set is named for the mechanic and never for one Hero's flavour, which a title carries instead. None of the three touches the Boss's health — a Downed Hero who could still push the kill is a Hero the Party has little reason to rescue. The card cost is what keeps the set honest against a state with no expiry: `reduce_escalation` is the only effect in the game that moves the clock backwards, and free and repeatable it would make getting knocked out the strongest play available.
_Avoid_: Ghost action, dead-player action, spectator mode

**Revive**:
The action that ends `Downed`, returning the Hero at the **authored `revive_to`** printed on the card that performs it — content rather than an engine constant, because it decides how forgiving the whole game is. Every card written today prints `1`. `rescue` is an `answer` Keyword, so a Revive is recognisable on a card nobody has seen before, and off-role reads as *this answer's suited Role is not this deck's Role*. The Healer's Revive is their Signature: fixed Slot, ranged, repeatable, charged by allies taking damage. Any other Role may author one at a premium from the fixed cost vocabulary — adjacent, one card discarded, the whole Round — because a run-ending problem must carry at least one off-role line (D-025) and `[HEALER ONLY]` is banned outright. Nothing about a Downed Hero is terminal, so there is no state a Revive cannot reach.
_Avoid_: Resurrection, free pickup, battle rez, exclusive revive
