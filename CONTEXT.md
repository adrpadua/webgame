...
**Fired Card**:
Everything that happens between a legal `fire_slot` and the ordered batch of generated actions it becomes: the Charge arithmetic over the stack and the Signature's token bank, the Reader verbs the fire executes (`scale` and `spend`; `gate` belongs to legality and refuses before the fire exists), the recipient and the overflow conversion, the burst geometry, the Counter placements, the raises, and the draw. One resolution concept with one fixed internal order — a cost `spend` is paid before the Charge arithmetic, `scale` reads what is left, a resolution `spend` comes off after the effects are computed — and D-096's one atomic terminal batch. The engine implements it as one module (`firedCard.ts`).
_Avoid_: Card effect pipeline, activation script
...
A Hazard is read on the `Tile Panel` a tap on its hex opens, as a `Status Icon` in the same tray a piece wears its Counters in.
...
**Stat Panel**:
The readout a tapped Enemy tile opens — the Boss or a Minion — floated over the board's lower edge. It carries the piece's health gauge and its `Status Icon`s; an icon shows its count whenever more than one is held, because a count the player cannot see is a count they cannot spend deliberately. The panel follows the piece rather than the hex and shows the staggered playout values while a Boss Row replays, so it reads as a live gauge; it closes from its own control, a tap on an empty hex, or a session transition, and stays up through ordinary play. Unlike a Detail Popup it persists instead of dismissing on release. Enemy-only since D-065: the Hero's readout is the Hero Frame, and tapping the Hero's tile pulses that frame instead of opening a panel here. Presentation only: it is never a rules surface.
_Avoid_: Tooltip, Detail Popup, HUD gauge, unit frame

**Status Icon**:
One live condition — a `Counter` held by a piece, or a `Hazard` held by ground — as a raked square rather than a word (D-088, D-091, party-frame direction 1A): the condition's mark cut in the material its channel names, on the same 8-degree plate every other HUD surface wears, with the count in the bottom-right corner whenever more than one is held and a clock across the foot while a Counter with an authored duration runs down. Icons stand in one tray beside the readout of whichever host is carrying them — the `Hero Frame` for the Hero, the `Stat Panel` for a Boss or a Minion, the `Tile Panel` for ground — where an MMO puts its buff tray, packed at 28pt with the tray as a whole taking the 44pt target and wrapping upward as it fills. Reading a mark is a glance; a hold opens the `Detail Popup` behind it. One target, two readings, decided by how exactly the input can point: a finger has the whole tray and gets every condition in it, each row led by the same square with its numbers and its authored rules text, while a pointer can name one square and gets that mark alone. Neither is a mode, and the two can never disagree — a single mark's reading is the same object either way. One tray rather than one control per Counter for the reason the `Hero Frame` is one hold target rather than four: a mark that owed its own 44pt would price a Party's afflictions in chrome the 390pt surface does not have. The mark is presentation keyed by content id and never authored in `data/`; anything nobody has drawn yet falls back to a neutral steel mark rather than failing to load. Counters and Hazards are marked from separate tables, and the two may deliberately share a mark — Scorched and Seared are the same fire on two hosts — because they never share a tray and the panel each stands on names its host. Presentation only: what a Counter does is the Counter's, and the square never changes it.
_Avoid_: Counter chip, buff icon, badge, status effect, debuff

**Tile Panel**:
The readout a tapped tile opens for the ground itself (D-091), beside the `Stat Panel` that reads the piece standing on it. It carries that hex's `Status Icon` tray — its `Hazard`s and any Counter the ground itself holds — and names the piece it is under when there is one, because coordinates are a debug affordance rather than the player's language for a tile. One bare tap opens whichever of the two readouts has something to say: burning ground under an Enemy is two subjects and two panels, and plain ground is a tap that closes both. The panel goes quiet the moment its ground does, so it never outlives its own subject, and it is the first dock member to yield when the lane fills — ground is the least urgent thing in it. Presentation only: it never decides what ground does.
_Avoid_: Tile tooltip, hex inspector, terrain popup

**Hero Frame**:
The primary Hero's persistent readout, built to unit-frame anatomy (D-065, ADR 0033): a left-justified vertical stack of the Hero's name, a dominant health bar carrying the Armor overlay, the `Class Resource` as a thinner bar directly beneath it, and the deck and discard counts. The resource bar is unlabelled — its position under the health bar is what names it. The `Status Icon` tray sits beside the frame rather than inside it, where an MMO puts its buffs, because a Counter is its own authored rule and the frame's own hold explains only the frame's numbers. The frame floats over the board's bottom edge without resizing the board, never dismisses, and is the notification dock's floor: transient prompts stack upward from its top edge. A tap on the Hero's tile pulses it rather than opening a Stat Panel; a hold anywhere on it opens one Detail Popup for the whole readout. It is read, never pressed — the `Signature Button` is what takes the press. Built as the party-frame seed: shaped so a second Hero's frame can sit beside it when the multi-Hero model lands, with only the primary Hero's built today. Presentation only: it is never a rules surface.
