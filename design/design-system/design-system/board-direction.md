<!-- Carried from docs/content/oathcraft-board-direction.md by design/design-system/build_ds.py.
...
The Guarded Front mark is ground, and sits at the bottom of the cool order for a reason worth stating: it is the one cool mark that is on the board for the whole Encounter. A persistent mark pitched at the destination step would be the loudest cool thing in every frame of every Round, and the destinations — which appear only while a card is in hand and are the thing being decided between — would be answering to it. So it is drawn as a line rather than a tint over the tile, and what separates it from the two brighter cool marks is shape: it traces the guarded hex's own outline, strong on the three edges facing the Boss and quiet on the three behind. Partial hex outlines are the board's vocabulary for a mark that has to sit in a frame already carrying brighter ones, which is also what keeps this mark off colour alone.

**Three warm marks now share one imminence step, and the third pays for it in shape.** A Minion's fuse (D-063) is telegraphed for the whole Round it burns through, and a blast landing in the `Incoming Row` is exactly as imminent as the cone burning in it — so it takes the cone's step rather than inventing a sixth warm value the ramp does not have. Colour cannot separate them, and size is why it must not try: a cone covers three hexes, while two Whelps with their fuses up cover ten of nineteen, and washing that at cone strength makes most of the arena the warning. So the blast splits the job. Its **outline** carries the warning, drawn once around the outside of the whole footprint at the token's own full chroma; the **wash** inside only says which ground is under it, pitched below the Boss sprite on purpose. The read is a boundary with a Whelp inside it rather than a set of hexes — which is also the shape of the answer, since crossing that line is one of the two ways to live through it.
