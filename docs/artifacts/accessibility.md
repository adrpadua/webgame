...
- **A known gap: movement has no keyboard path.** The direction pad that provided one was removed — it read as a list of hexes duplicating the board, and it spent a strip of the play surface restating what the board already paints. Movement is now pointer-only: drag a card to a hex, drag the Hero to a hex, or tap a card then tap a hex. All three end on the same docked prompt, which names the destination in words and offers `Stay put`, and every drag destination still has a tap path as the pointer clause above requires. What is not satisfied is keyboard reach: the board is a canvas with no focusable hexes, so a keyboard user can prepare, charge, fire, undo, and advance, but cannot step. Restoring it means making the hexes themselves focusable — the board is the thing that should answer the keyboard, not a control beside it repeating its contents. The smoke suite asserts on the portrait canvas that nothing docked covers the strip below the bottom hex row, so the destinations a player is being asked to step to stay tappable.
