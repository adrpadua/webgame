...
- Detail Popups never hide required information behind a gesture alone. Every hold target keeps its own visible name, number, and state on the HUD, carries an `aria-label` that states the same thing, and meets the `44` pixel target minimum. The hold gesture has a keyboard equivalent: holding `Enter` or `Space` on the focused control opens the popup on the repeat event and releasing closes it, so a quick press stays an ordinary activation.
