...
**The dock's floor is the Hero Frame (D-058).** The frame is persistent chrome, not a notification — it never comes or goes, so it has no rank to claim — and the dock's column stacks upward from the frame's top edge instead of the Action Bar's. The smoke suite holds every floating member off the frame the same way it holds them off the bands.
