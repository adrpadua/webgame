...
| 14 | A fixed Hero power that does not depend on the draw ([proposal](design-proposals/fixed-hero-power.md), [research](research/2026-08-19-fixed-hero-powers-and-command-zones.md)) | Every authored demand is answered by the shuffle first: each 2-of in the live deck is in hand on ~37% of Rounds, and 43% of runs contain a 4+ Round drought on a given answer, which Escalation then charges for. The Hero Design Contract's Recovery row has no real answer today. | Awaiting a product decision. Then an ADR (the first exception to ADR 0008's unconditional full-charge cleanup), two flags on `SlotState`, a Signature card per Hero in `data/`, and the deck-evaluation cohort under D-016. |
