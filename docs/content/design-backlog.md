...
| 14 | The Signature Slot — a Hero's engine given a visible, authorable home ([proposal](design-proposals/fixed-hero-power.md), [research](research/2026-08-19-fixed-hero-powers-and-command-zones.md)) | A Hero's engine has nowhere to live. Riposte Ready — the rule that most defines how Elian plays — is hard-coded in `counters.ts`/`resolve.ts` with Elian's constants in TypeScript (D-033), so it is invisible on the bar, unauthorable in `data/`, and needs its own code branch for every future Hero. Downstream, the deck is the only source of capability: each 2-of is in hand on ~37% of Rounds and 43% of runs carry a 4+ Round drought, which Escalation then charges for. | Awaiting a product decision. Then an ADR (the first exception to ADR 0008's unconditional full-charge cleanup, plus the D-033 revisit), two flags on `SlotState`, a two-clause Signature schema in `data/`, and the Riposte migration as the first increment — measured against the existing cohort, with an explicit battery test that the two normal Slots keep firing (ADR 0002's Slot Tension). |
