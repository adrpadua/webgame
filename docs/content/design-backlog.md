...
| 14 | The Signature Slot — a Hero's engine given a visible, authorable home ([proposal](design-proposals/fixed-hero-power.md), [research](research/2026-08-19-fixed-hero-powers-and-command-zones.md)) | A Hero's engine has nowhere to live. Riposte Ready — the rule that most defines how Elian plays — is hard-coded in `counters.ts`/`resolve.ts` with Elian's constants in TypeScript (D-033), so it is invisible on the bar, unauthorable in `data/`, and needs its own code branch for every future Hero. Downstream, the deck is the only source of capability: each 2-of is in hand on ~37% of Rounds and 43% of runs carry a 4+ Round drought, which Escalation then charges for. | **Design settled 2026-08-19** (twelve designer-approved decisions in the proposal): per-Hero two-clause Signature; earned charge only (the standing clause is the sole Charge source — this answers the battery risk by construction); the Riposte Ready Counter, `Shield Slam`, and D-015 all retire; charges bank to `max_charge: 2`; `Iron Guard` to 8 fills the freed slots; migration numbers are Slam's own, with the `7`-ceiling caveat flagged for the cohort. The Character Design Bible carries the ⏳-marked revision. Remaining: the formal `D-0xx` entry, the ADR (ADR 0008 exception, D-015 retirement, D-033 revisit), then the migration increment and its evaluation cohort under D-016. |
