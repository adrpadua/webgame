...
- **Burst from a Card**: a Card with `"target_type": "hex"`,
  ...
  `damage_to_target` modifiers increase each Enemy's damage, not the radius
  (ADR 0030).
...
- **Charge Modifier effects.** `armor_gain`, `health_restored`,
  `damage_to_boss`, `damage_to_target` — each names the Card quantity it
  raises (D-056). Counter Readers use their own vocabulary: `armor_gain`,
  `damage_taken`, `damage_dealt`, `damage_to_boss`.
