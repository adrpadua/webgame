...
| Counter: `data/counters/` | Identity, optional `keywords`, `host` (`combatant`, `hex`, or `slot` — D-046), `max` (`1` refuses a second placement; higher accumulates — a placement that would overflow lands what fits and the fact records how many, so `fortified`'s `30` is a rail well above the twelve two Slow Slots can bank rather than a design cap), `duration_rounds` (`0` = no clock), and `readers`. A Counter has no payload of its own: each Reader names `when` (`round_start`, `host_damage_incoming`, `host_deals_damage`, `slot_fired`, and on a `hex` host `host_entered`), an `effect`, a signed `per` applied once per Counter held, and optionally one `event_keyword` narrowing a damage Reader to blows carrying that Damage Keyword — so Sundered is `host_damage_incoming`/`target_damage`/`1` and Weakened is `host_deals_damage`/`target_damage`/`-1`, and neither declares a side (D-045). A Reader is a **modifier**, read before the blow resolves and upstream of Armor: `host_damage_incoming` is the blow arriving at the host, `host_deals_damage` the blow leaving. The reaction moment — `host_takes_damage`, read after the blow lands — belongs to Signature earns alone (D-085), and the event registry (ADR 0041) refuses either word on the wrong side at load, naming the file. A Card places it through `places_counter` and `counter_amount`; where it lands comes from that Card's `target_type`, which must be able to supply the Counter's host — `none`/`piece` for `combatant`, `hex` for `hex`, `board_slot` for `slot`. Ground outlives whoever stands on it; a Slot's Counters ride its prepared Top Card and are dropped when that Slot is re-loaded. A Reader's event must happen to its host: a `combatant` hears the blow-and-clock events, a `hex` hears exactly one — `host_entered` (D-086), dealing its `per` per Counter to whoever steps on it, a walker paying every crossed hex and a jumper only its landing — and a `slot` hears none. A Counter nothing can read fails the build. |
...
| Boss: `data/bosses/` | Identity, positive `max_health`, and complete rules text. Deliberately thin: a Boss's substance lives in its Programs, its terrain rules on its Hazards, its movement on its Beats. The id doubles as the board entity id, so it never changes for a display name (ADR 0040). |
| Minion: `data/minions/` | Identity, positive `max_health`, `attack_damage`, and complete rules text, plus how it fights and travels: `range_tiles` for the bite, `move_tiles` for the allowance, `standoff_tiles` for how close it is trying to stand, and `traversal` (D-072, D-079). The Standoff may not sit outside the bite reach — that Minion would creep into position and never attack. Boss Beats reference the Minion they spawn. New Minion triggers require engineering. |
| Encounter: `data/encounters/` | The `party` it fields — an ordered list of seats, each naming a `hero` by id from `data/heroes/`, a `start` hex, an optional per-seat `deck` (empty falls back to `player_deck`), and optional `fields_signature: false` to hold that Hero's Signature back as the teaching slice does (ADR 0035). One to four seats; a solo fight is a Party of one. A seat's own deck is what states that Hero's Role, so two seats sharing one deck are two seats with the same Role. The `boss` it fields, by id from `data/bosses/` (ADR 0040). `board_radius`, legal starts, `slot_count`, `hand_refill_target`, `revive_health_fraction` (what a Revived Hero returns on, as a fraction of maximum rounded up — `0.25` by default, and authored because it decides how forgiving the fight is), complete `player_deck`, ordered `boss_programs`, `loop_boss_programs`, `round_limit` and `enrage_text`, `random_seed`, `minion_spawn_candidates`, optional `phase_trigger` + `phase_two_programs` + `phase_break_text` (ADR 0023), and optional `escalation_thresholds` (ADR 0027). No new file to register — it appears in the Workbench's Encounter list on save. |
...
- **A Hero and their Signature**: `data/heroes/elian.json` is Elian —
  `title` is the first name every readout prints, `full_name` carries
  `Captain Elian Voss` as flavour, and `elian_riposte` is named as the printed
  `signature_card` beside `max_health`.
  `data/encounters/embermaw_first_turn.json` fields them with
...
- **Boss Beat kinds.** `turn_toward_player`, `advance_toward_player`,
  ...
  `damage`, `hazard`, `minion`, `count`, `range_tiles`, `move_tiles`,
  `standoff_tiles` — is an
...
- **Signature earn events.** All five: `host_takes_damage`,
  ...
  (D-085): a Grant cannot change the number, and authoring one there is
  refused at load.
