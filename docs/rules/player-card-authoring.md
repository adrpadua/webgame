...
| `places_counter` | A Counter id from `data/counters/` (D-033, D-045). Where it lands follows `target_type`, which must be able to supply the Counter's host (D-046): `none` or `piece` for a `combatant` Counter, `hex` for a `hex` Counter, `board_slot` for a `slot` Counter. Place an existing Counter rather than authoring a near-duplicate — a second Sundered with different text is how a shared vocabulary stops being shared. |
| `damage_keywords` | What this card's damage is made of and who it is aimed at, as registered `damage_type` Keyword ids (D-047). A Counter Reader can narrow itself to one of these, so keywording a card's damage is what lets a Counter answer it. A card declaring these must actually deal damage. |
