...
| `range_tiles` | How far the card reaches, in hexes from the firing Hero to whatever it lands on (D-070). One number for the whole card: it answers the selected piece, the selected hex, forced movement, *and* the Boss a `boss_damage` card never names. Author `1` for a melee swing. A card that touches anything past its own Hero must carry a reach, and a card that touches nobody must not — the build refuses both. `board_slot` is the exception: an ally's Top Card is not a place on the board, and support is adjacency-free (D-009). |
