...
| No blame-sink | The healer must not be sized to cover everything (triage is the skill), and their prevention must be visible: Encounter Records and the HUD credit damage prevented and windows covered, the way the Tank's earned Riposte is credited. The zero-health model carries the other half of this principle, and D-079 (ADR 0040) sharpened it in two ways. A failed save now costs a `Downed` ally and nothing worse — there is no expiry and no terminal state, so the healer's hardest decision is never whether a teammate stops playing, and there is no *moment* at which they failed to save someone. And rescue is never healer-only: the universal `revive_ally` means every Role holds the expensive line, so a party that loses its healer is dealt a severe blow rather than a wall. That is this principle at the structural level — a healer who is the single point of failure for everyone's recovery is a blame-sink by construction, whatever the interface credits. | A failed Round produces a readable shared lesson, not a default verdict of "the healer was slow." A party that has lost its healer can still name what it is going to do about the body on the floor. |
