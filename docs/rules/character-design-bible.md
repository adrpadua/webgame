...
| No blame-sink | The healer must not be sized to cover everything (triage is the skill), and their prevention must be visible: Encounter Records and the HUD credit damage prevented and windows covered, the way the Tank's earned Riposte is credited. The zero-health model carries the other half of this principle: a failed save costs a `Downed` ally and then an `Incapacitated` one, never an eliminated player (D-068, ADR 0036), so the healer's hardest decision is never whether a teammate stops playing. | A failed Round produces a readable shared lesson, not a default verdict of "the healer was slow." |
