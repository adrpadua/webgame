...
| Working names | One verb phrase per ability, stating its whole effect with no theme in it, recorded beside the flavour name and kept there. Flavour is applied last, after the mechanic is settled — see [Name It For What It Does](player-card-authoring.md#name-it-for-what-it-does). | No working name needs an `and` or an `or`, no two abilities in the kit produce the same one, and none of them reads flat. A design doc whose abilities are only named in flavour has not been reviewed yet — it has been admired. |
...
| No blame-sink | The healer must not be sized to cover everything (triage is the skill), and their prevention must be visible: Encounter Records and the HUD credit damage prevented and windows covered, the way the Tank's earned Riposte is credited. The zero-health model carries the other half of this principle: a failed save costs a `Downed` ally and then an `Incapacitated` one, never an eliminated player (D-070, ADR 0036), so the healer's hardest decision is never whether a teammate stops playing. | A failed Round produces a readable shared lesson, not a default verdict of "the healer was slow." |

The Second Hero Of A Role rule applies to healers from the first one: the Restorative (undo landed damage, remove Boss-placed marks, convert deliberate overheal) and the planned Enchanter (prevent through wards bound to named Beats) must not converge — their one-page statements are written together in [healer-archetype-statements.md](../content/heroes/healer-archetype-statements.md) precisely so the boundary is drawn before either deck exists. A Catcher-style kit (zones, denial, lockdown) is not the Enchanter's Healer sibling: under our Role Contract its verbs preserve nobody, so it belongs to a control-flavored Damage or future Controller discussion, not this table. The same sibling rule applies to future Damage Heroes. Both siblings must still pass the same Role recognition test; what differs is the machine, its signature weakness, and the pressure it answers best.
...
| Payoff | Fire *Riposte*: spend all Signature Charges for `2` Boss damage `+3` per Charge (D-101); fired at the cap of `2` it also **Sunders** the Boss for the Round — cash one for tempo, or bank to two when follow-up hits are waiting to land through the wound. Elsewhere: absorb the hit that would break the line, or clear a nearby Minion. |
