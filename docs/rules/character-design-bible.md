...
Two authoring rules. Reuse the target kind's existing rule rather than inventing one — every Enemy, the Boss included, must be within `range_tiles` (D-067), and an ally's card needs no adjacency. And check the catalog before writing a new status: if a status with the meaning you want exists, apply that one, because a second Sundered with different text is how a vocabulary stops being shared.
