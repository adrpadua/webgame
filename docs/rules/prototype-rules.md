...
With a Party, defeat is the whole Party being `Downed` at once rather than any one Hero falling (D-079, ADR 0040). A Hero at `0` becomes `Downed` — a blocking, non-targetable body who answers no demand and satisfies no Role selector, whose Slots and Charges freeze and whose hand stops refilling — and stays that way until `Revived` by an adjacent ally discarding a card, which returns them at the Encounter's authored `revive_to`. There is no window and no terminal state. Each Round a Downed player spends one card from hand on one of three ally-facing actions (`grant_ally_armor`, `ally_draws_card`, `reduce_escalation`), so nobody is ever out of the game. The solo slice above is unaffected without needing a rule of its own: a Party of one that falls has every seat Downed, which is the defeat condition.
