...
// (D-057). It answers one event from the same closed `when` set, narrowed by
...
// Riposte's Sundered rider (D-057 decision 13). The Counter lands on the
...
export const cardSchema = z.object({
  ...
  // A Signature card (D-057, ADR 0032): printed on a Hero, never in the deck,
...
export const encounterSchema = z.object({
  ...
  // The Hero's Signature (D-057): a `fixed: true` card installed as an
