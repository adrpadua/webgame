...
export function createEncounterState(catalog: ContentCatalog, encounterId: string, seedOverride?: number): EncounterState {
  ...
  // The Hero definition (ADR 0034): identity, health pool, and the printed
  // Signature all ride it. The catalog has already validated the reference.
  const heroDefinition = catalog.heroes[encounter.hero]
  if (!heroDefinition) {
    throw new Error(`Encounter ${encounterId} fields unknown hero ${encounter.hero}`)
  }
  const signatureCard = encounter.fields_signature ? heroDefinition.signature_card : ''
  ...
  addEntity(
    ...
    heroDefinition.max_health,
    FACING_NE,
    'party',
    heroDefinition.title,
  )
