...
export function BeatCard() {
  ...
  const found = nextBeatId === null ? null : findLiveBeat(catalog, state, nextBeatId)
    ...
    >
      <span className="flex items-baseline justify-between gap-2">
        <span className="shrink-0 text-[9px] font-semibold tracking-widest text-steel-400 uppercase">
          {found ? (found.track === 'instant' ? 'Boss · Instant' : 'Boss · Incoming') : 'Up next'}
        </span>
        ...
        <span className="shrink-0 text-[11px] font-black tracking-widest text-coral-300 uppercase">Resolve ▸</span>
      </span>
      <span className="truncate text-sm font-bold text-coral-100">{nextBeatTitle ?? 'Boss beat'}</span>
      {stats.length > 0 && (
      ...
      {found && found.beat.rules_text !== '' && <span className="text-[10px] leading-snug text-steel-400">{found.beat.rules_text}</span>}
      {found && found.beat.answer_tags.length > 0 && (
        // Content stores Keyword ids; the card shows their authored titles, so
        // renaming an answer's wording never means editing five program files.
  ...
  return (
    ...
    >
      <span className="flex min-w-0 items-baseline gap-2">
        <span className="shrink-0 text-[9px] font-semibold tracking-widest text-steel-400 uppercase">Standing</span>
        <span className="truncate text-xs font-bold text-coral-100">{beat.title}</span>
      </span>
      <span className={`shrink-0 text-[10px] font-black tracking-widest uppercase ${answered ? 'text-gold-200' : 'text-coral-300'}`}>
        {answered ? 'Answered' : `Escalation +${beat.escalation_if_unanswered}`}
      </span>
    </div>
  )
}
