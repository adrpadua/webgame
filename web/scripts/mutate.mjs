...
const MUTATIONS = [
  ...
  },
  {
    ...
    guards: 'D-NEW: impassable is physics, and physics does not care whose move it is',
    file: 'engine/board.ts',
  ...
  },
  {
    ...
    guards: 'D-NEW: arriving is not moving, and a piece still cannot be where a piece cannot be',
    file: 'engine/board.ts',
  ...
  },
  {
    ...
    guards: 'D-NEW: a telegraph the player invalidates by playing correctly is ADR 0031 all over again',
    file: 'engine/content/catalog.ts',
    from: '      if (beatMoves(beat)) {\n        throw new Error(',
    to: '      if (false && beatMoves(beat)) {\n        throw new Error(',
  },
  {
    ...
    guards: 'D-055 / D-NEW: a field that changes an outcome and never reaches the card is a port defect',
