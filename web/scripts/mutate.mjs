...
const MUTATIONS = [
  ...
  },
  {
    ...
    guards: 'D-074: range_tiles is what tells a movement clause when to stop',
    file: 'engine/content/catalog.ts',
  ...
  },
  {
    ...
    file: 'engine/resolve.ts',
    from: "      if (struck && struck.status === 'living' && struck.health <= 0) {",
    to: '      if (false) {',
  },
  {
    name: 'a parked corpse answers the proximity demand',
    guards: 'ADR 0039 / D-041: only a living Hero answers a demand',
    file: 'engine/escalation.ts',
    from: '      return !livingHeroIds(state).some((heroId) => {',
    to: '      return !Object.keys(state.heroes).some((heroId) => {',
  },
  {
    name: 'a Downed hand refills, so the diminished actions are free forever',
    guards: 'ADR 0039: the hand a Hero fell with is the whole budget on the floor',
    file: 'engine/advancePhase.ts',
    from: "        if (draft.heroes[heroId].status !== 'living') {\n          continue\n        }",
    to: '',
  },
  {
    name: 'a diminished action costs nothing',
    guards: 'ADR 0039: free reduce_escalation on a state with no expiry beats every real play',
    file: 'engine/legality.ts',
    from: "        return illegal('A diminished action costs a card from hand.')",
    to: '        return legal()',
  },
  {
    name: 'the revive health is hardcoded instead of authored',
    guards: 'ADR 0039: how forgiving the fight is must be reachable by a designer',
    file: 'engine/resolve.ts',
    from: 'encounter?.revive_to ?? 1',
    to: '1',
  },
  {
